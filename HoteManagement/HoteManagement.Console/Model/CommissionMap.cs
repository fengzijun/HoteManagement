//------------------------------------------------------------------------------
// <auto-generated>
//     此代码已从模板生成。
//
//     手动更改此文件可能导致应用程序出现意外的行为。
//     如果重新生成代码，将覆盖对此文件的手动更改。
// </auto-generated>
//------------------------------------------------------------------------------
using System;
using System.Collections.Generic;

namespace HoteManagement.Console.Model
{
    
    
    public class CommissionMap : BaseEntityTypeConfiguration<HoteManagement.Domain.Commission>
    {
       public CommissionMap()
       {
          this.HasKey(t => t.Id);
          this.Property(t => t.Accounts).HasMaxLength(50);
          this.Property(t => t.CommDesp).HasMaxLength(50);
          this.Property(t => t.CommDate);
          this.Property(t => t.CommSum);
          this.Property(t => t.IsBack);
          this.Property(t => t.GoodNumber).HasMaxLength(50);
          this.Property(t => t.IsEveryDay);
          this.Property(t => t.DayComm);
          this.Property(t => t.CommRemark).HasMaxLength(50);
          this.Property(t => t.createtime);
          this.Property(t => t.updatetime);
       }
    }
}
